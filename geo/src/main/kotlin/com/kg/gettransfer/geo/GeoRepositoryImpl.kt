...
class GeoRepositoryImpl(private val context: Context) : GeoRepository {
        ...
        locationProviderClient.lastLocation
            ...
            .addOnFailureListener {
                cont.resume(
                    Result(
                        Point(),
                        ApiException(ApiException.NOT_FOUND, it.message ?: "Unknown")
                    )
                )
            }
    }
    ...
    override fun getAddressByLocation(point: Point, pair: Pair<Point, Point>): Result<GTAddress> {
        val list = try {
            ...
            return Result(GTAddress.EMPTY, ApiException(ApiException.GEOCODER_ERROR, e.message ?: "Unknown"))
        }
    ...
    override fun getCurrentAddress(): Result<GTAddress> {
        ...
        return try {
            ...
            if (list.isEmpty()) Result(GTAddress.EMPTY, ApiException(ApiException.NOT_FOUND, "Address not found"))
                ...
                val cityPoint =
                    CityPoint(place.name.toString(), Point(place.latLng.latitude, place.latLng.longitude), place.id)
                Result(GTAddress(cityPoint, place.placeTypes, place.address.toString(), null, null))
            }
        } catch (e: Exception) {
            Result(GTAddress.EMPTY, ApiException(ApiException.NETWORK_ERROR, e.message ?: "Unknown"))
        }
    }
    ...
    override fun getAutocompletePredictions(prediction: String, points: Pair<Point, Point>?): Result<List<GTAddress>> {
        ...
        return try {
            ...
            Result(list.map {
                GTAddress(
                    CityPoint(it.getPrimaryText(null).toString(), null, it.placeId),
                    it.placeTypes,
            ...
            Result(emptyList<GTAddress>(), ApiException(ApiException.NETWORK_ERROR, e.message ?: "Unknown"))
        }
    }
    ...
    override fun getLatLngByPlaceId(placeId: String): Result<Point> {
        ...
        return try {
            ...
            Result(Point(), ApiException(ApiException.NETWORK_ERROR, e.message ?: "Unknown"))
        }
    }
}
