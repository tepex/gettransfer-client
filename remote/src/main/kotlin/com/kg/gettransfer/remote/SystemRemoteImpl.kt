...
class SystemRemoteImpl(private val api: Api,
    ...
    override suspend fun getConfigs(): ConfigsEntity {
        val response: ResponseModel<ConfigsModel> = core.tryTwice { api.getConfigs() }
		return configsMapper.fromRemote(response.data!!)
    }
    
    override suspend fun getAccount(): AccountEntity {
        val response: ResponseModel<AccountWrapperModel> = core.tryTwice { api.getAccount() }
    ...
    private suspend fun tryPutAccount(account: AccountModel): ResponseModel<AccountWrapperModel> {
        return try { api.putAccount(account).await() }
        catch(e: Exception) {
            ...
            return try { api.putAccount(account).await() } catch(e2: Exception) { throw core.remoteException(e2) }
        }
    }
    ...
    private suspend fun tryLogin(email: String, password: String): ResponseModel<AccountWrapperModel> {
        return try { api.login(email, password).await() }
            ...
            try { core.updateAccessToken() } catch(e1: Exception) { throw core.remoteException(e1) }
            return try { api.login(email, password).await() } catch(e2: Exception) { throw core.remoteException(e2) }
        }
    }
}
