...
class ChildSeatsFragment : BaseBottomSheetFragment(), ChildSeatsView, KoinComponent {
    ...
    private fun performPlus(type: Int) = delegate.increase(type, this)

    private fun performMinus(type: Int) = delegate.decrease(type, this)

    override fun updateView(count: Int, type: Int) {
        val res = if (count == 0) minusDisabled else minusEnabled
        context?.let { ctx ->
            getMinusImageView(type).setImageDrawable(ContextCompat.getDrawable(ctx, res))
            val seatImageRes = SeatImageProvider.getImage(type, count > 0)
            getSeatImage(type).setImageDrawable(ContextCompat.getDrawable(ctx, seatImageRes))
        }
        getTextView(type).text = "$count"
    }

    private fun getMinusImageView(type: Int) = when (type) {
    ...
    private fun getTextView(type: Int) = when (type) {
    ...
    private fun getSeatImage(type: Int) = when (type) {
